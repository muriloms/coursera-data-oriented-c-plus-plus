...
void computeGrid(auto& particles, auto& grid) {
    ...
    // <<<CODE HERE>>>
    // Use the sort algorithm with a lambda that compare the particles by the index of their position array.
    sort( ... );
    ...
    for_each(policy, begin(ids), end(ids), [&particles, &grid](auto i) {
        //<<<CODE HERE>>>
        // complete this lambda function to update the `grid` vector 
        // according to the lecture (directory vector).
        // This part is quite hard, do not hesitate to experiment
        // with small grids and prints all results.
    } );
}
